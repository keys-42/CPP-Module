...
int
Rpn::sumNumber(int x, int y) const {
	return y + x;
}
int
Rpn::differenceNumber(int x, int y) const {
	return y - x;
}
int
Rpn::productNumber(int x, int y) const {
	return y * x;
}
int
Rpn::quotientNumber(int x, int y) const {
	return y / x;
}
		...
		if (arr[i] == c) {
			if (this->isStackEmpty())
				throw std::logic_error(
						"logic error: The provided expression is not in RPN.");
			x = this->stackTop();
			this->stackPop();
			if (this->isStackEmpty())
				throw std::logic_error(
...
void
checkInput(const std::string& line) {
	std::string s = "+-*/";

	for (std::size_t i = 0; i < line.size(); ++i) {
		if ((i % 2) == 0) {
			if ((isdigit(line[i]) == true) ||
				(s.find(line[i]) != std::string::npos)) {
				continue;
			} else {
...
Rpn::rpn(const std::string& line) {
	try {
		if (line.empty())
			throw std::invalid_argument(
					"Error: The provided RPN expression is empty.");
		checkInput(line);
	...
	try {
		...
		if (!this->isStackEmpty()) {
			throw std::runtime_error(
					"Invalid input: The provided expression is not in RPN.");
		}
