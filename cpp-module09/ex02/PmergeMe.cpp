...
void PmergeMe::printClock(std::clock_t start, std::clock_t end, E_Type type) {
  ...
  std::cout << "Time to process a range of " << this->size_
            << " elements with std::[" << t << "] : " << std::fixed
            << std::setprecision(5) << microseconds << " us" << std::endl;
}
  ...
  for (int i = 0; i < size; ++i) {
    ...
    if (sorted_.find(numbers[i]) != sorted_.end())
      throw std::runtime_error(
          "The number already exists in the sorted container.");
...
size_t PmergeMe::calculateJacobsthalNumber(size_t n) {
  if (n <= 0) return 0;
  if (n == 1) return 1;
  return calculateJacobsthalNumber(n - 1) +
         (2 * calculateJacobsthalNumber(n - 2));
}
 ...
 */
// pair comparison
bool PmergeMe::shouldSwapPairs(const int l, const int r) const { return l < r; }
...
void PmergeMe::processPairs(std::list<int>& lst, std::list<int>& subChain,
                            int pairSize) {
...
void PmergeMe::splitIntoMainAndSubChains(std::list<int>& mainChain,
  ...
  while (true) {
    std::list<int>::iterator segmentStart =
        getIteratorAt(mainChain, segmentIndex * pairSize);
    ...
    subChain.splice(subChainInsertPosition, mainChain, segmentStart,
                    segmentEnd);
...
int PmergeMe::lower_bound(std::list<int>& mainChain, int key, int pairSize,
                          int endpoint) {
  std::list<int> lst;
...
void PmergeMe::insertSubChain(std::list<int>& mainChain, int insertPosition,
...
void PmergeMe::insertSegmentToMainChain(std::list<int>& mainChain,
                                        ...
                                        int insertionPoint, int pairSize) {
  std::list<int>::iterator startIter =
      getIteratorAt(subChain, segmentStart * pairSize);
  std::list<int>::iterator endIter =
      (segmentEnd == -1) ? subChain.end()
                         : getIteratorAt(subChain, segmentEnd * pairSize);
  mainChain.insert(getIteratorAt(mainChain, pairSize * insertionPoint),
                   startIter, endIter);
}
...
void PmergeMe::mergeSubIntoMain(std::list<int>& mainChain,
  ...
  for (size_t n = 3;; ++n) {
    for (size_t i = calculateJacobsthalNumber(n);
         i > calculateJacobsthalNumber(n - 1); --i) {
      if (i > maxSegments) {
        i = maxSegments;
      ...
      int insertionPoint =
          lower_bound(mainChain, key, pairSize, insertSize + i - 1);
#ifdef INSERT
#ifdef LISTDEBUG
      ...
      insertSegmentToMainChain(mainChain, subChain, i - 1, i, insertionPoint,
                               pairSize);
  ...
  if (!tmp.empty()) {
    int key = *(tmp.begin());
    int insertionPoint =
        lower_bound(mainChain, key, pairSize, mainChain.size() / pairSize);
...
// pair comparison
bool PmergeMe::shouldSwapPairs(const std::vector<int>& vec, int pairSize,
                               int startIndex) {
  return getElementAtIndex(vec, startIndex) <
         getElementAtIndex(vec, startIndex + pairSize);
}
...
bool PmergeMe::isPairPresent(const std::vector<int>& vec, int pairSize,
                             size_t startIndex) {
  return getConstIteratorAt(vec, startIndex + 2 * pairSize - 1) != vec.end();
}

void PmergeMe::processPairs(std::vector<int>& vec, std::vector<int>& subChain,
                            int pairSize) {
  for (size_t i = 0; i < vec.size();) {
    ...
    if (shouldSwapPairs(vec, pairSize, i)) {
      swapPairs(vec, vec.begin() + i, vec.begin() + i + pairSize,
                vec.begin() + i + pairSize, vec.end());
    }
...
int PmergeMe::lower_bound(std::vector<int>& mainChain, int key, int pairSize,
                          int endpoint) {
  std::vector<int> vec;
...
void PmergeMe::insertSubChain(std::vector<int>& mainChain, int insertPosition,
...
void PmergeMe::mergeSubIntoMain(std::vector<int>& mainChain,
  ...
  if (!tmp.empty()) {
    ...
    int insertionPoint =
        lower_bound(mainChain, key, pairSize, mainChain.size() / pairSize);
    mainChain.insert(mainChain.begin() + pairSize * insertionPoint, tmp.begin(),
                     tmp.end());
  }
#ifdef INSERT
  std::cout << std::endl << std::endl;
