...
class PmergeMe {
 ...
 private:
  ...
  // // insert
  void mergeSubIntoMain(std::list<int>& mainChain, std::list<int>& subChain,
                        int pairSize, std::list<int>& tmp);
  int lower_bound(std::list<int>& mainChain, int key, int pairSize,
                  int endpoint);
  void createComparisonListFromMainChain(std::list<int>& lst,
                                         std::list<int>& mainChain,
                                         int pairSize, int endpoint);
  bool elementExceedsKey(std::list<int>& lst, int index, int key);
  int find_lower_bound(std::list<int>& lst, int key);
  void insertSubChain(std::list<int>& mainChain, int insertPosition,
                      IntListIterator begin, IntListIterator end);
  void insertSegmentToMainChain(std::list<int>& mainChain,
                                std::list<int>& subChain, int segmentStart,
                                int segmentEnd, int insertionPoint,
                                int pairSize);
   ...
   */
  // processPairs
  bool shouldSwapPairs(const std::vector<int>& vec, int pairSize,
                       int startIndex);
  void swapPairs(std::vector<int>& vec, std::vector<int>::iterator leftStart,
                 std::vector<int>::iterator leftEnd,
                 std::vector<int>::iterator rightStart,
                 std::vector<int>::iterator rightEnd);
  bool isPairPresent(const std::vector<int>& vec, int pairSize,
                     size_t startIndex);
  void processPairs(std::vector<int>& vec, std::vector<int>& subChain,
                    int pairSize);

  // splitIntoMainAndSubChains
  void splitIntoMainAndSubChains(std::vector<int>& mainChain,
                                 std::vector<int>& subChain, int pairSize);
  ...
  // // insert
  void mergeSubIntoMain(std::vector<int>& mainChain, std::vector<int>& subChain,
                        int pairSize, std::vector<int>& tmp);
  int lower_bound(std::vector<int>& mainChain, int key, int pairSize,
                  int endpoint);
  ...
  void insertSubChain(std::vector<int>& mainChain, int insertPosition,
                      IntVecIterator begin, IntVecIterator end);
  void insertSegmentToMainChain(std::vector<int>& mainChain,
                                std::vector<int>& subChain, int segmentStart,
                                int segmentEnd, int insertionPoint,
                                int pairSize);
...
#ifdef SEPARATE
#ifdef LISTDEBUG
    testSeparateDebug(mainChain, subChain, pairSize,
                      "Before split into main and sub-chains.");
#endif
#ifdef VECTORDEBUG
    testSeparateDebug(mainChain, subChain, pairSize,
                      "Before split into main and sub-chains.");
...
#ifdef LISTDEBUG
    testSeparateDebug(mainChain, subChain, pairSize,
                      "After split into main and sub-chains.");
#endif
#ifdef VECTORDEBUG
    testSeparateDebug(mainChain, subChain, pairSize,
                      "After split into main and sub-chains.");
...
#ifdef FIRST
#ifdef LISTDEBUG
    testFirstInsertDebug(mainChain, subChain, pairSize,
                         "Before prepend sub-chain to main.");
#endif
#ifdef VECTORDEBUG
    testFirstInsertDebug(mainChain, subChain, pairSize,
                         "Before prepend sub-chain to main.");
...
#ifdef LISTDEBUG
    testFirstInsertDebug(mainChain, subChain, pairSize,
                         "After prepend sub-chain to main.");
#endif
#ifdef VECTORDEBUG
    testFirstInsertDebug(mainChain, subChain, pairSize,
                         "After prepend sub-chain to main.");
 ...
 private:
  template <typename T>
  typename std::vector<T>::iterator getIteratorAt(std::vector<T>& vec,
                                                  size_t index) {
    if (index >= vec.size()) {
      return vec.end();
    }
  ...
  template <typename T>
  typename std::list<T>::iterator getIteratorAt(std::list<T>& lst,
                                                size_t index) {
    if (index >= lst.size()) {
      return lst.end();
    }
  ...
  template <typename T>
  typename std::vector<T>::const_iterator getConstIteratorAt(
      const std::vector<T>& vec, size_t index) {
    if (index >= vec.size()) {
      return vec.end();
    }
  ...
  template <typename T>
  typename std::list<T>::const_iterator getConstIteratorAt(
      const std::list<T>& lst, size_t index) {
    if (index >= lst.size()) {
      return lst.end();
    }
  ...
  template <typename T>
  void advanceTo(std::vector<T>& vec, typename std::vector<T>::iterator& itr,
                 size_t index) {
  ...
  template <typename T>
  void retreatBy(std::vector<T>& vec, typename std::vector<T>::iterator& itr,
                 size_t index) {
  ...
  template <typename T>
  void advanceTo(std::list<T>& lst, typename std::list<T>::iterator& itr,
                 size_t index) {
  ...
  template <typename T>
  void retreatBy(std::list<T>& lst, typename std::list<T>::iterator& itr,
                 size_t index) {
    for (size_t i = 0; i < index; ++i) {
      if (itr == lst.begin()) return;
      --itr;
    }
    return;
  }
  ...
  template <typename T>
  typename std::vector<T>::iterator getAdvanceTo(
      std::vector<T>& vec, typename std::vector<T>::iterator& itr,
      size_t index) {
    if (itr + index < vec.end()) {
      itr += index;
    } else {
  ...
  template <typename T>
  typename std::list<T>::iterator getAdvanceTo(
      std::list<T>& lst, typename std::list<T>::iterator& itr, size_t index) {
    for (size_t i = 0; i < index; ++i) {
 ...
 public:
  ...
  template <typename T>
  void testInsertDebug(T main, T sub, int pairSize, std::string s, int key,
                       int jacoNum) {
    typedef typename T::iterator Iter;
    int i = 0;
    if (s.compare(0, 6, "Before", 0, 6) == 0) {
      ...
      std::cout << BLUE << "[" << RESET << "n: " << jacoNum << " key: " << key
                << BLUE << "]" << RESET;
    }
  }
};
