  ...
  for (;;) {
    ...
    if (all_digits)
      ...
      std::cout << phone_number_
                << " is not valid. Please enter only numerical values."
                << std::endl
                << std::endl;
  }
...
void Contact::getContants() {
  std::cout << std::setw(10) << changeContent(first_name_) << "|"
            << std::setw(10) << changeContent(last_name_) << "|"
            << std::setw(10) << changeContent(nickname_) << "|" << std::endl;
}
