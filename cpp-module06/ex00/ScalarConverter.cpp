...
void ScalarConverter::convert(std::string const &str) {
  LiteralType types[] = {
      CHAR, INT, FLOAT, DOUBLE, PSEUDO,
  };

  typedef void (*FuncPtr)(const std::string &);
  FuncPtr fcPtr[] = {
      ScalarConverter::char_literal,   ScalarConverter::int_literal,
      ScalarConverter::float_literal,  ScalarConverter::double_literal,
...
void ScalarConverter::int_literal(std::string const &str) {
  ...
  if (str_float.find('e') == std::string::npos &&
      str_float.find('E') == std::string::npos) {
    str_float += ".0f";
  } else {
    str_float += "f";
  }

  if (str_double.find('e') == std::string::npos &&
      str_double.find('E') == std::string::npos) {
    str_double += ".0";
  }
...
void ScalarConverter::double_literal(std::string const &str) {
  ...
  if (number == static_cast<int>(number)) {
    std::cout << "flaot  : " << static_cast<float>(number) << ".0f"
              << std::endl;
